...
- Cloudflare does not control the length of an SSH, VNC, or RDP session. [Application session durations](/cloudflare-one/identity/users/session-management/) only determine the window in which a user can initiate a new connection.
