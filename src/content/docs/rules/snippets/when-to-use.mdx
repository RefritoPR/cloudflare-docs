...
- Runs after cache evaluation – unlike Workers, which always execute before cache, Snippets trigger after [Cache Rules](/cache/how-to/cache-rules/). This means you can cache response content modified by Snippets, allowing subsequent visitors to receive the pre-processed response.
...
| Return cached response content before execution | ✅ | ❌ |
...
export default {
  async fetch(request) {
      ...
      url.pathname = "info.php";
      
      const newRequest = new Request(url, request);
      const customKey = `https://${url.hostname}${url.pathname}`;
...
export default {
    async fetch(request) {
        ...
        if (!response) {
            ...
            url.hostname = "worker.example.com";
